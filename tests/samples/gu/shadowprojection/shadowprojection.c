...
void initGu(void* frameBuffer, void* doubleBuffer, void* renderTarget, void* depthBuffer)
{
	sceGuInit();
...
void renderOrbitingLight(int* val)
{
...
int main(int argc, char* argv[])
{
	...
	// setup GU
	initGu(frameBuffer, doubleBuffer, renderTarget, depthBuffer);
	setupMatrices()
	// define shadowmap

	Texture shadowmap = { GU_PSM_4444, 0, 128, 128, 128, sceGeEdramGetAddr() + (int)renderTarget };

	// define geometry
	// Geometry torus = { identity, sizeof(torus_indices)/sizeof(unsigned short), torus_indices, torus_vertices, 0xffffff };
	Geometry grid = { identity, sizeof(grid_indices)/sizeof(unsigned short), grid_indices, grid_vertices, 0xff7777 };
